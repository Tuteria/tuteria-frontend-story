...
@hosts("sama@tutor-search.tuteria.com")
def build_store():
    with cd("/home/sama/tuteria-store"):
        run("git pull --no-edit")
        ...
        run("docker login -u gbozee -p abiola2321 registry.gitlab.com")
        run(
            "docker build --no-cache -t registry.gitlab.com/tuteria/v2/tuteria-store ."
        )
        run("docker push registry.gitlab.com/tuteria/v2/tuteria-store")
    run('docker rmi $(docker images --filter "dangling=true" -q --no-trunc)')
